...
Tier 1 had ingest write straight to SQLite. Tier 2 splits that in two:
this module still does the async WebSocket listening, but now it
publishes each Tick onto the Redis stream instead of touching storage
directly. Storage becomes the workers' job, not ingest's - ingest's
only responsibility is "get ticks off the wire and onto the queue."
"""
import asyncio
import logging
...
async def run_producer(symbols: list[str], seconds: int, redis_host: str = "localhost"):
    """
    Listen to the live feed for a fixed window, publishing every tick to
    the Redis stream. Mirrors Tier 1's collect_ticks, just swapping the
    SQLite write for a queue publish.
    """
    client = get_redis_client(host=redis_host)
    count = 0
